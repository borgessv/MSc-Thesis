...
def make_dataset(model_param, seed=None, samples=20, **kwargs):
    ...
    if model == "ROM":
        ...
        phi_r = eng.create_rom(beam_data,n_modes,DoF,Xeq,disp_progress,nargout=1)
    else:
        phi_r = []
    ...
    if model == 'FOM':
        sio.savemat(beam_data[:-5]+'_'+model+str(n_DoF)+'_s'+str(samples)+'_'+str(t0)+'-'+str(dt)+'-'+str(t1)+'.mat', data_raw)
    else:
        sio.savemat(beam_data[:-5]+'_'+model+str(n_modes)+'_s'+str(samples)+'_'+str(t0)+'-'+str(dt)+'-'+str(t1)+'.mat', data_raw)
