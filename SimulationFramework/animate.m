function animate(beam_data,sol_file,models,DoF,n,varargin)
global beam
...
if any(strcmp(varargin,'gif'))
    gif_filename = [beam_data(1:end-5), '.gif'];
...
% end
mymap = [0.45 0.45 0.45
    0 0.75 0
    0 0 1
    1 0 0
    1 1 1];
cm = colormap(mymap(1:length(models),:));
for i = 1:length(T)
    f = figure('visible','off');
    %f.WindowState = 'maximized';
    f.Position = [150, 42, 950, 645];
    ...
    for j = 1:length(models)
        X = load(sol_file,append('X_',models{j}));
        X = X.(append('X_',models{j}));
        q = X(length(T)+1:end,:);
        ...
        if models{j} == "FOM" || models{j} == "ROM"
            surf(ax,X3D_def,Y3D_def,Z3D_def,'facecolor',cm(j,:),'linestyle','none','FaceAlpha',0.5);
        else
            surf(ax,X3D_def,Y3D_def,Z3D_def,'facecolor',cm(j,:),'linestyle','none');
        end
            hold on
        plot3(ax,X3D_def(:),Y3D_def(:),Z3D_def(:),'color',[0.5 0.5 0.5]);
%         if i == 1 && j == 1
%             % Instant Position:
%             fig1 = surf(ax,X3D_def,Y3D_def,Z3D_def,'facecolor',cm(j,:),'linestyle','none');
%             hold(ax,'on')
...
%         end
        ...
        lgd_entry{2*j-1} = models{j};
        lgd_entry{2*j} = '';
    end
    if lgd_entry{1} == "FOM" || lgd_entry{1} == "ROM"
        lgd_entry{1} = append(lgd_entry{1},strcat(" (n = ",num2str(n),")"));
    end
    title(ax,['$t =$ ',num2str(T(i),'%.3f'),' s'],'interpreter','latex','fontsize',10)
